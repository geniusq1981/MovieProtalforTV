...
class ImgFigure extends React.Component {
  ...
  render() {
    ...
    let capClassName = "img-title";
    capClassName += this.props.arrange.isFocused ? ' title-show' : '';
    ...
    return (
      <figure className={imgFigureClassName} style={styleObj} onClick={this.handleClick}>
        ...
        />
        <figcaption>
          <h2 className={capClassName}>{this.props.data.title}</h2>
        </figcaption>
      </figure>
    )
  }
...
class AppComponent extends React.Component {
  constructor(props) {
    ...
    this.constant = {
      ...
      focus: {
        area: 0,    //0:top area;1:content area;2:bottom area;
        position: 0  //
      }
    };
  }
  ...
  componentDidMount() {
    ...
    this.rearrange(0);
    //注册按键事件
    document.body.addEventListener('keyup', this.bodykeypress).bind(this);
  ...
  render() {
    ...
    imageDatas.forEach(function (value, index) {
      ...
      controllerUnits.push(<ControllerUnit key={index} arrange={this.state.imgArrangeArr[index]}
                                           inverse={this.inverse(index)} focus={this.focus(index)}/>);
    }.bind(this));
    return (
      <section className="stage" ref="stage">
        <section className="top-sec">
          <div className="logo">照 片 墙</div>
          <div className="clock">10 点</div>
        </section>
